export async function register() {
  ...
  const { landscapeGraph, landscapeNodeCount, ...counts } = await seedFromDataDir(store, dataDir)
  if (counts.pathways === 0)
    throw new Error(`no pathways loaded from ${dataDir} — check working directory / data dir`)
  console.log('[seed]', { ...counts, landscapeGraphNodes: landscapeNodeCount, hasLandscape: landscapeGraph !== undefined })
